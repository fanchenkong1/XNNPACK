...
WASMSIMD=--cpu=wasm  --features=wasm_relaxed_simd --features=wasm_simd --crosstool_top=@emsdk//emscripten_toolchain:everything --spawn_strategy=local --host_crosstool_top=@bazel_tools//tools/cpp:toolchain
...
x8_lut_bench:
	$(BAZEL) build  $(PROXY) $(VERBOSE) $(DEFINE) -c opt  $(WASMSIMD) x8_lut_bench 2>&1 | tee log2.log

x8_lut_test:
	$(BAZEL) build  $(PROXY) $(VERBOSE) $(DEFINE) -c opt  $(WASMSIMD) x8_lut_test 2>&1 | tee log2.log

f32_vsigmoid_test:
	$(BAZEL) build  $(PROXY) $(VERBOSE) $(DEFINE) -c opt  $(WASMSIMD) f32_vsigmoid_test 2>&1 | tee log2.log

qd8_f32_qc8w_gemm_minmax_test:
	$(BAZEL) build  $(PROXY) $(VERBOSE) $(DEFINE) -c opt  $(WASMSIMD) qd8_f32_qc8w_gemm_minmax_test 2>&1 | tee log2.log

qd8_f32_qc8w_gemm_bench:
	$(BAZEL) build  $(PROXY) $(VERBOSE) $(DEFINE) -c opt  $(WASMSIMD) qd8_f32_qc8w_gemm_bench 2>&1 | tee log2.log

qd8_f32_qc8w_igemm_minmax_test:
	$(BAZEL) build  $(PROXY) $(VERBOSE) $(DEFINE) -c opt  $(WASMSIMD) qd8_f32_qc8w_igemm_minmax_test 2>&1 | tee log2.log
